...
export function ExperimentalLogo({ 
  trigger = "none", 
  color = "#000000",
  className = ""
}: { 
  trigger?: "forward" | "reverse" | "none";
  color?: string;
  className?: string;
}) {
  ...
  useEffect(() => {
    ...
    // If none, we just ensure it's in a base state on first load
    else if (trigger === "none" && !timelineRef.current) {
