 ...
 * `path` draws as an outline stroke; `fillPath` fades in to fill the glyph.
 */
const letters = [
  {
    key: "o1",
...
// Gentle wobble while the outline draws — keeps the original hand-drawn feel.
const drawWobbleEase = (t: number) => t - Math.sin(t * 4 * Math.PI) / (4 * Math.PI);

interface ExperimentalLogoProps {
...
export function ExperimentalLogo({
  ...
  useEffect(() => {
    ...
    // Prime the hidden starting state once — further triggers animate from
    // whatever the previous timeline left on the DOM.
    ...
    // Kill any running timeline before kicking off a new one.
    timelineRef.current?.kill();

    if (trigger === "forward") {
      // Draw strokes, then fade fills in with a 1s overlap.
      ...
      // REVERSE — fade fills out while strokes retract (last → first).
      const tl = gsap.timeline();
      timelineRef.current = tl;
